...
---------------------------------------------------------------
	function f1(a, b : in vector; ma, me : in matrix) return vector is
	begin 
		return sort(a) * (ma * me) + sort(b);
	end;
	function f2(mk, mh, mf : in matrix) return matrix is
	begin 
		return transpose(mk) * (mh * mf);
	end;
	function f3(r, s : in vector; mt, mp : in matrix) return vector is
	begin
		return sort(r + s) * (mt * mp);
	end;
